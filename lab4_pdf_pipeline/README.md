...
3. Создать систему генерации векторных эмбеддингов с помощью Mistral API (`mistral-embed`, 1024 dim).
...
- Библиотеки: `docling`, `lancedb`, `mistralai`, `tiktoken`, `tenacity`
...
3. **Создание эмбеддингов**: Используйте Mistral API (`mistral-embed`) для генерации эмбеддингов размерности 1024.
4. **Векторная база данных**: Настройте LanceDB для эффективного хранения и поиска векторов.
5. **Обработка ошибок API**: Реализуйте механизм повторных попыток при ошибках API с экспоненциальным откатом (`tenacity`).
